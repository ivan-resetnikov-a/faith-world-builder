...
class FaithWorldBuilder :
	...
	def update (self) :
		...
		if self.mousePressed[1] :
			...
			self.objects.append(core.Object(part['name'], self.mousePos))
	...
	def render (self) :
		...
		if self.showInterface : self.frame.blit(self.parts[self.selected]['img'], self.mousePos)
		...
		# objects menu
		if self.mousePos[1] < 5 :
			self.menuY += (0 - self.menuY) * 0.3
		else :
			self.menuY += (-25 - self.menuY) * 0.3

		pg.draw.rect(self.frame, (100, 100, 100), (0, self.menuY, 250, 25))

		x = 3
		for obj in self.parts :
			self.frame.blit(obj['img'], (x, self.menuY+6))
			x += obj['img'].get_width() + 3
		################
	...
	def writeLevel (self) :
		...
		objects = []
		for obj in self.objects : objects.append({'name': obj.name, 'pos': obj.pos})
